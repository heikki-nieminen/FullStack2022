...
const summaParilliset = () => {
  ...
  return taulukko.reduce((edellinen, nykyinen) =>
    nykyinen % 2 == 0 ? edellinen + nykyinen : edellinen
  )
}
...
const luoElementit = () => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    elementit.push(elementinTilavuus * elementinTiheys)
  ...
  return elementit.sort((a, b) => {
    return b - a
  })
}

let elementtiLista = luoElementit()
...
const laskeKuormat = () => {
  ...
  while (elementtiLista.length > 0) {
    if (kuorma < 10500) {
      if (mahtuuko) {
        let listanPituus = elementtiLista.length
        for (let i = 0; i <= listanPituus - 1; i++) {
          if (kuorma + elementtiLista[i] <= 10500) {
            kuorma += elementtiLista[i]
            elementit.push(elementtiLista[i])
            elementtiLista.splice(i, 1)
            i--
            mahtuuko = true
          } else {
            mahtuuko = false
        ...
        console.log(
          "Kuorma numero: " +
            lastit +
            "   Kokonaispaino: " +
            elementit.reduce((acc, item) => acc + item)
        )
      } else {
        lastit++
        kuorma = 0
